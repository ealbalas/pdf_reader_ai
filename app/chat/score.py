...
def random_component_by_score(component_type, component_map):
    """
    Randomly selects a component from the provided component map based on its score.

    :param component_type: 'llm', 'retriever', or 'memory'
    :param component_map: A dictionary mapping component names to their scores.

    From redis get hash containing the sum total scores and number of times each component has been scored

    :return: A random component name from the component map.
    """
    if component_type not in ['llm', 'retriever', 'memory']:
        raise ValueError(f"Invalid component type: {component_type}")
    
    # Get the total scores and counts from Redis
    scores = client.hgetall(f"{component_type}_score")
    ...
    for name in names:
        ...
        average_scores[name] = max(avg_score, 0.1)
...
def score_conversation(
    ...
    client.hincrby("llm_score", llm, score)
    client.hincrby("llm_score_counts", llm, 1)
    client.hincrby("retriever_score", retriever, score)
    client.hincrby("retriever_score_counts", retriever, 1)
    client.hincrby("memory_score", memory, score)
...
def get_scores():
    ...
    for component_type in ['llm', 'retriever', 'memory']:
        ...
        for name in names:
            score = int(scores.get(name, 1))
            count = int(counts.get(name, 1))
            aggregate_scores[component_type][name] = [score / count]
